...
sub fetch_data {
   my $comp = $db->get_collection('PlacesFoursquare');
    my $addr = $comp->find({});
    $addr->immortal(1);
    my $count = 1;
    ITERATOR: while (my $cursor = $addr->next) {
        my $addr = $cursor->{location}->{formattedAddress}; 
        next if @{$addr} < 3;
        foreach my $index (1 .. 4) {
            next unless $addr->[$index];
            if ($addr->[$index] eq 'São Paulo, SP') {
               last; 
            }
            next ITERATOR;
        }
        my $res = clean_addr($addr->[0]);
        next if !$res;
        my $cksum = cksum(@{$res});
